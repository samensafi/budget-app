...
RULES:
1. Find every transaction (purchases, payments, refunds, transfers, fees). Skip running balances, statement totals, interest summaries, and any other figure that is not an individual transaction. Each transaction is ONE line item with a real merchant/payee name, and its amount is the one printed on THAT line, beside THAT name. A number that has no real merchant/payee name printed beside it is NOT a transaction, no matter where it sits or what it represents (a date- or section-heading total, a daily net like "+$132" next to "Friday, Jun 05", a subtotal, or just a stray figure that happens to sit near the others). Do NOT attach such a number to any transaction: never shift it onto a neighboring line, never copy a nearby merchant's name onto it to create a second row, and never invent a transaction to soak it up. If a number has no name of its own, drop it. A standalone phone number, reference/confirmation number, or store number sitting on or beside a merchant line (e.g. "STARBUCKS 8007927282") is part of THAT merchant's descriptor, never split it into a second transaction, and never output a transaction whose merchant is only digits / a phone number.
